...
\section{Overall Description}
	...
	\subsection{Product Perspective}
		...
		Obstacles described previously such as poles and low hanging roofs,intersections on campus(between IT Bulding and EB building)  will be fitted with fiducial markers. These codes will be used to notify users of a certain area of obstacles. when in a certain proximity to those obstacles.\par
		The system will use the user's smart device's hardware interfaces such as WiFi and GPS to accurately get the user's location data, and navigate them to their required destination. The system wil also use fiducial markers detected by machine vision on the smart devices to capture information sent by obstacles so as to notify the user of its proximity.\\
